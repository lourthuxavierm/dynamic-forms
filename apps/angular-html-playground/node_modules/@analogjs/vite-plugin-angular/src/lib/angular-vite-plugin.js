...
import { createFilter, defaultClientConditions, normalizePath, preprocessCSS, } from 'vite';
...
import { TS_EXT_REGEX, createTsConfigGetter, getTsConfigPath, createDepOptimizerConfig, isProdMode, } from './utils/plugin-config.js';
...
export function angular(options) {
    ...
    function angularPlugin() {
        ...
        return {
            ...
            configureServer(server) {
                ...
                const invalidateCompilationOnFsChange = createFsWatcherCacheInvalidator(invalidateFsCaches, invalidateTsconfigCaches, () => performCompilation(resolvedConfig), pluginOptions.include.map((glob) => `${normalizePath(resolve(pluginOptions.workspaceRoot))}${glob}`));
            ...
            transform: {
                ...
                async handler(code, id) {
                    ...
                    const resolveResourceUrls = code.includes('@Component') && (watchMode || jit);
                    ...
                    if (jit && data.includes('angular:jit:')) {
                        ...
                        data = data.replace(/angular:jit:style:inline;([A-Za-z0-9+/=]*)/g, (_match, encodedStyles) => toJitInlineStyleId(encodedStyles));
    ...
    async function performAngularCompilation(config, ids) {
        ...
        const { errorsByFile, warningsByFile, globalErrors, globalWarnings } = groupDiagnosticsByFile(diagnostics);
    ...
    async function _doPerformCompilation(config, ids) {
        ...
        const writeOutputFile = (id) => {
            ...
            if (emittedIds.has(normalizedId) &&
            ...
            builder.emit(sourceFile, (filename, data) => {
                ...
                if (!watchMode &&
                    ...
                    const declarationPath = resolve(config.root, config.build.outDir, relativeToRoot).replace('/src/', '/');
...
export function createFsWatcherCacheInvalidator(invalidateFsCaches, invalidateTsconfigCaches, performCompilation, includeGlobs = [], debounceMs = 100) {
...
export function groupDiagnosticsByFile(diagnostics) {
    ...
    const group = (list, byFile, global) => {
        for (const diagnostic of list ?? []) {
            const location = diagnostic.location;
            const message = location?.file
                ? `${location.file}:${location.line ?? 0}:${location.column ?? 0}: ${diagnostic.text ?? ''}`
                : (diagnostic.text ?? '');
            if (location?.file) {
 ...
 */
export function collectEmittedDiagnostics(outputFiles) {
    const errors = [];
    const warnings = [];
 ...
 */
export function formatDiagnosticWithLocation(diagnostic) {
    const text = ts.flattenDiagnosticMessageText(diagnostic.messageText, '\n');
    if (!diagnostic.file || diagnostic.start == null) {
        return text;
    }
    const { line, character } = diagnostic.file.getLineAndCharacterOfPosition(diagnostic.start);
    return `${normalizePath(diagnostic.file.fileName)}:${line + 1}:${character + 1}: ${text}`;
}
            ...
            if (!cached) {
                ...
                for (const node of sourceFile.statements) {
                    if (ts.isClassDeclaration(node) && node.name != null) {
                        cached.hmrUpdateCode = angularCompiler?.emitHmrUpdateModule(node);
                        if (!!cached.hmrUpdateCode) {
                            cached.className = node.name.getText();
 ...
 */
export function isAngularCompilationFile(code, hasEmittedContent) {
