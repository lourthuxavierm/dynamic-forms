...
import { compile, scanFile, scanPackageDts, collectImportedPackages, collectRelativeReExports, collectAllImports, jitTransform, inlineResourceUrls, extractInlineStyles, generateHmrCode, debugCompile, debugRegistry, ANGULAR_DECORATOR_CALL_RE, } from './compiler/index.js';
import { TS_EXT_REGEX, getTsConfigPath, createDepOptimizerConfig, isProdMode, } from './utils/plugin-config.js';
...
export function fastCompilePlugin(pluginOptions) {
     ...
     */
    async function scanBarrelExports(file, visited = new Set(), overwrite = false, fileContents, scanResults) {
        if (visited.has(file))
            return;
        visited.add(file);
        let code;
    ...
    async function initFastCompile() {
            ...
            for (const [key, targets] of Object.entries(tsPaths)) {
                ...
                if (key.endsWith('/*')) {
                    ...
                    if (spec.startsWith(prefix)) {
                        ...
                        return list.map((t) => resolve(baseUrl, t.split('*').join(spec.slice(prefix.length))));
                    }
                }
                else if (key === spec) {
        ...
        const baseResolveCache = new Map();
        ...
        const importWalkVisited = new Set();
        ...
        const bareSpecResolveCache = new Map();
        const walkImports = async (file) => {
            if (importWalkVisited.has(file))
                return;
            ...
            for (const pkg of collectImportedPackages(code, file, program)) {
                ...
                try {
                    for (const entry of scanPackageDts(pkg, projectRoot, scannedDtsPackages)) {
                        if (!registry.has(entry.className))
                            registry.set(entry.className, entry);
                    }
                }
            ...
            const targets = await Promise.all(collectAllImports(code, file, program).map((spec) => {
                if (spec.startsWith('.')) {
                    return resolveToSource(resolve(dir, spec.replace(/\.(?:js|mjs)$/u, '')));
                }
