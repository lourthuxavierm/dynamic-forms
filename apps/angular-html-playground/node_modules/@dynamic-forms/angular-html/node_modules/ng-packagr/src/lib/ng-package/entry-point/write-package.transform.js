...
const writePackageTransform = (options) => (0, transform_1.transformFromPromise)(async (graph) => {
    ...
    const ngPackageNode = (0, nodes_1.findPackageNode)(graph);
    ...
    if (!ngEntryPoint.isSecondaryEntryPoint) {
        try {
            ...
            if (!options.watch) {
                ...
                const nestedPackageJsons = [];
                // Secondary entry-points package.json
                for (const { data } of entryPoints) {
                    if (!data.entryPoint.isSecondaryEntryPoint) {
                        continue;
                    }
                    const packageJsonPath = (0, path_1.ensureUnixPath)(path.join(path.relative(primary.destinationPath, data.entryPoint.destinationPath), 'package.json'));
            ...
            await writePackageJson(ngEntryPoint, ngPackage, {
                ...
                exports: (0, package_json_1.generatePackageExports)(ngEntryPoint, entryPoints),
...
async function writePackageJson(entryPoint, pkg, additionalProperties, isWatchMode, compilationMode, spinner) {
    ...
    if (isWatchMode) {
        ...
        packageJson.version = (0, package_json_1.generateWatchVersion)();
    }
