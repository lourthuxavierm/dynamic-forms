...
async function rollupBundleFile(opts) {
    ...
    if (dtsMode) {
        outExtension = '.d.ts';
        plugins = [(0, file_loader_plugin_1.fileLoaderPlugin)(opts.fileCache, ['.d.ts', '/index.d.ts'], dtsMode), (0, rollup_plugin_dts_1.dts)({ sourcemap: opts.sourcemap })];
    }
    else {
        outExtension = '.mjs';
        plugins = [(0, file_loader_plugin_1.fileLoaderPlugin)(opts.fileCache, ['.js', '/index.js'], dtsMode), (0, plugin_json_1.default)()];
    }
    // Create the bundle
    const bundle = await rollup.rollup({
        context: 'this',
        external: (moduleId, parentId) => isExternalDependency(moduleId, parentId, jail),
        cache: opts.cache ?? (cacheDirectory ? await (0, cache_1.readCacheEntry)(cacheDirectory, opts.cacheKey) : undefined),
        input: opts.entry,
        plugins,
        onwarn: warning => {
            switch (warning.code) {
