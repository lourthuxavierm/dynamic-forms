...
const watchTransformFactory = (project, options, analyseSourcesTransform, entryPointTransform) => (source$) => {
    ...
    return source$.pipe((0, rxjs_1.switchMap)(graph => {
        const { data, cache: { sourcesFileCache }, } = (0, nodes_1.findPackageNode)(graph);
        ...
        scheduleEntryPoints(entryPointTransform, options), (0, rxjs_1.repeat)({ delay: () => (graph.some((0, nodes_1.isEntryPointPending)()) ? (0, rxjs_1.of)(1) : rxjs_1.EMPTY) }), (0, rxjs_1.last)(), (0, rxjs_1.tap)(() => printBuiltAngularPackage(ngPkg, startTime)), (0, rxjs_1.catchError)(error => {
            log.error(error);
...
const buildTransformFactory = (project, options, analyseSourcesTransform, entryPointTransform) => (source$) => {
    return source$.pipe((0, rxjs_1.switchMap)(graph => {
        const startTime = Date.now();
        const pkgUri = (0, nodes_1.ngUrl)(project);
        const ngPkg = graph.get(pkgUri);
        ...
        // Next, run through the entry point transformation (assets rendering, code compilation)
        scheduleEntryPoints(entryPointTransform, options), (0, rxjs_1.tap)(() => printBuiltAngularPackage(ngPkg, startTime)));
    }));
};
const scheduleEntryPoints = (epTransform, options) => (0, rxjs_1.pipe)((0, rxjs_1.concatMap)(graph => {
