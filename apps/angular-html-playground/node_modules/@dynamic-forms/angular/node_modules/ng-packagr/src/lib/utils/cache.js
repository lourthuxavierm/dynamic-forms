...
async function readCacheEntry(cachePath, key) {
    try {
        const data = await (0, promises_1.readFile)(getCacheFilePath(cachePath, key), 'utf8');
...
async function saveCacheEntry(cachePath, key, content) {
    ...
    return (0, promises_1.writeFile)(getCacheFilePath(cachePath, key), data, 'utf8');
}
function getCacheFilePath(cachePath, key) {
    return (0, node_path_1.join)(cachePath, `${key}.json`);
}
