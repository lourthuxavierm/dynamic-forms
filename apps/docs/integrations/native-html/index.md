...
`@dynamic-forms/html` package.

The shipped renderer is [React HTML](../../packages/react-html.md), which uses
browser-native elements through React. `@dynamic-forms/html` is only a
...
| --- | --- |
| Browser-native controls in a React application | Use `@dynamic-forms/react-html` |
| Existing code importing `@dynamic-forms/html` | Migrate to `@dynamic-forms/react-html` |
| Direct DOM rendering with no React dependency | Not available; integration is Planned |
| Framework-independent form state without a renderer | Use `@dynamic-forms/core` |
