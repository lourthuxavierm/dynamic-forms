...
| Core | Framework-independent schemas, state, validation, conditions, dependencies, data sources, registries, and events in `@dynamic-form-engine/core`. | Do not attribute rendering or framework lifecycle to Core. |
...
| Headless React | React context, hooks, subscriptions, and renderer-neutral components in `@dynamic-form-engine/react`. | It does not provide the default browser controls or stylesheet. |
| React HTML | The shipped `@dynamic-form-engine/react-html` renderer. It renders browser-native HTML elements through React. | Preferred name for the current renderer. |
...
| HTML compatibility package | `@dynamic-form-engine/html`, which forwards `@dynamic-form-engine/react-html`. | Do not call it a separate renderer or recommend it for new applications. |
...
| Avoid | Prefer |
| --- | --- |
| “the HTML package” when the package is unclear | `@dynamic-form-engine/react-html` or `@dynamic-form-engine/html` |
