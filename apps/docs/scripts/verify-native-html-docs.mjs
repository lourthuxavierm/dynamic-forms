...
if (packageManifest.description !== 'Compatibility package forwarding to @dynamic-form-engine/react-html') {
  failures.push('packages/html/package.json: compatibility description changed; review Native HTML docs');
}
if (packageManifest.dependencies?.['@dynamic-form-engine/react-html'] !== 'workspace:*') {
  failures.push('packages/html/package.json: expected direct @dynamic-form-engine/react-html dependency');
}
for (const peer of ['@dynamic-form-engine/core', '@dynamic-form-engine/react', 'react', 'react-dom']) {
  if (!packageManifest.peerDependencies?.[peer]) failures.push(`packages/html/package.json: missing ${peer} peer dependency`);
}
if (packageIndex !== "export * from '@dynamic-form-engine/react-html';") {
