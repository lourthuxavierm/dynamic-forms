...
if (manifest.name !== '@dynamic-form-engine/zod') failures.push('Zod package name mismatch');
if (manifest.dependencies?.['@dynamic-form-engine/core'] !== 'workspace:*') failures.push('Zod adapter must depend on Core');
if (manifest.peerDependencies?.zod !== '^3.25.5 || ^4.0.0') failures.push('Zod peer matrix must be ^3.25.5 || ^4.0.0');
for (const forbidden of ['@dynamic-form-engine/react', '@dynamic-form-engine/react-html', '@dynamic-form-engine/angular', '@dynamic-form-engine/angular-html']) {
...
for (const command of ['typecheck', 'test', 'build']) {
  if (!matrix.includes(`@dynamic-form-engine/zod ${command}`)) failures.push(`Zod matrix missing ${command} command`);
}
