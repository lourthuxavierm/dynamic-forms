...
test('API overview separates generated signatures from handwritten guidance', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: '@lourthuxavierm/dynamic-forms-core' })).toHaveAttribute('href', /generated\/core$/);
...
test('generated Core API contains curated and source-derived entries', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: '@lourthuxavierm/dynamic-forms-core API' })).toBeVisible();
...
test('generated Zod API exposes Release-ready maturity and validator factories', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: '@lourthuxavierm/dynamic-forms-zod API' })).toBeVisible();
