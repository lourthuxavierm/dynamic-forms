...
test('layout gallery switches responsive patterns, RTL, sticky actions, and custom registration', async ({ page }) => {
  await page.goto('http://127.0.0.1:4175/layouts');
  await expect(page.getByRole('heading', { level: 1, name: 'Enterprise layout gallery' })).toBeVisible();
  await expect(page.getByText('Twelve-column form grid', { exact: false })).toBeVisible();
  await page.getByLabel('Layout pattern').click();
  await page.getByRole('option', { name: 'Sections and fieldsets' }).click();
  await expect(page.locator('fieldset').filter({ hasText: 'Identity' })).toBeVisible();
  await page.getByLabel('Layout pattern').click();
  await page.getByRole('option', { name: 'Cards, tabs and accordions' }).click();
  await expect(page.getByRole('tab', { name: 'Profile' })).toBeVisible();
  await expect(page.getByText('Additional details')).toBeVisible();
  await page.getByLabel('Layout pattern').click();
  await page.getByRole('option', { name: 'Sidebar and detail' }).click();
  await expect(page.getByRole('button', { name: 'Organization' })).toBeVisible();
  await page.getByLabel('Layout pattern').click();
  await page.getByRole('option', { name: 'Read-only summary' }).click();
  await expect(page.getByText('Full name:')).toBeVisible();
  await page.getByLabel('Mobile preview').check();
  await expect(page.getByTestId('layout-preview')).toContainText('390px mobile preview');
  await page.getByLabel('RTL layout').check();
  await expect(page.getByTestId('layout-preview')).toHaveAttribute('dir', 'rtl');
  await expect(page.getByLabel('Sticky action bar')).toBeVisible();
  await page.getByRole('button', { name: 'Save layout demo' }).click();
  await expect(page.getByText('Layout demo saved.')).toBeVisible();
  await page.getByRole('button', { name: 'Register custom layout' }).click();
  await expect(page.getByText('Runtime-registered two-column compact layout.')).toBeVisible();
  await expect(page.getByText('Registered: compact layout available')).toBeVisible();
  await expect(page.getByLabel('Custom layout contract')).toContainText("registry.register('compact'");
});

test('enterprise showcase completes onboarding, loan, and admin cross-feature workflows', async ({ page }) => {
  await page.goto('http://127.0.0.1:4175/showcase');
  await expect(page.getByRole('heading', { level: 1, name: 'Enterprise workflow showcase' })).toBeVisible();
  await page.getByRole('button', { name: 'Async company lookup' }).click();
  await expect(page.getByText('Company verified')).toBeVisible();
  ...
  await expect(page.getByLabel('GST tax ID')).toBeVisible();
  await page.getByRole('button', { name: 'Review onboarding' }).click();
  await expect(page.getByRole('heading', { name: 'Onboarding review' })).toBeVisible();
  await page.getByRole('button', { name: 'Submit customer' }).click();
  await expect(page.getByText('Customer onboarding submitted.')).toBeVisible();
  await page.getByRole('tab', { name: 'Loan application' }).click();
  await expect(page.getByText('Calculated financed amount: USD 400,000')).toBeVisible();
  await page.getByRole('button', { name: 'Next loan step' }).click();
  await page.getByRole('button', { name: 'Add co-applicant' }).click();
  await expect(page.getByLabel('Co-applicant 1')).toBeVisible();
  await page.getByRole('button', { name: 'Next loan step' }).click();
  await expect(page.getByText('Senior approval required', { exact: false })).toBeVisible();
  await page.getByLabel('Reviewer role').click();
  await page.getByRole('option', { name: 'Senior approver' }).click();
  await expect(page.getByText('active review role: senior', { exact: false })).toBeVisible();
  await page.getByRole('tab', { name: 'Admin configuration' }).click();
  await page.getByLabel('Tenant locale').fill('fr-FR');
  await expect(page.getByText('Draft/dirty protection: unsaved changes')).toBeVisible();
  await page.getByRole('button', { name: 'Load async policy options' }).click();
  await expect(page.getByText('3 policy templates loaded')).toBeVisible();
  await page.getByRole('button', { name: 'Save admin draft' }).click();
  await expect(page.getByText('Draft/dirty protection: saved')).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Audit event stream' }).locator('..')).toContainText('Draft saved');
  await expect(page.getByLabel('Redacted admin configuration')).toContainText('[REDACTED]');
  await expect(page.getByLabel('Redacted admin configuration')).not.toContainText('secret-token');
});
