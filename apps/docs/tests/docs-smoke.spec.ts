...
test('loads the documentation home and navigates to the React HTML package', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('heading', { level: 1, name: 'Dynamic Forms documentation' })).toBeVisible();

  await page.getByRole('link', { name: 'React HTML' }).first().click();
  await expect(page).toHaveURL(/\/packages\/react-html$/);
  await expect(page.getByRole('heading', { level: 1, name: '@dynamic-forms/react-html' })).toBeVisible();
});
