...
export function FieldNode(props: NodeProps) {
  ...
  return <div className="node-wrap" role="treeitem" aria-selected={props.selectedPath === path}>
    <article className={props.selectedPath === path ? 'field-node selected' : 'field-node'} data-field-path={path} tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); props.onSelect(path); } }} onClick={() => props.onSelect(path)} draggable onDragStart={(event) => { event.stopPropagation(); event.dataTransfer.setData('application/x-builder-path', path); }}>
