...
export function reducer(state: BuilderState, action: Action): BuilderState {
  ...
  if (action.type === 'patch-field') {
    const schema = updateField(state.schema, action.path, action.patch);
    const selectedPath = action.patch.name ? [...action.path.split('.').slice(0, -1), action.patch.name].join('.') : action.path;
    return { ...state, schema, selectedPath, past: [...state.past.slice(-49), state.schema], future: [], saved: false, message: action.message };
  }
