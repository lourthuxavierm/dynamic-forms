...
const NestedFields = lazy(() => import('./pages/NestedFields/NestedFields'));
...
function Shell() {
  ...
  const page = route?.id === 'dashboard' ? <Dashboard /> : route?.id === 'basic-form' ? <BasicForm /> : route?.id === 'fields' ? <Fields /> : route?.id === 'validation' ? <Validation /> : route?.id === 'conditions' ? <Conditions /> : route?.id === 'dependencies' ? <Dependencies /> : route?.id === 'data-sources' ? <DataSources /> : route?.id === 'arrays' ? <Arrays /> : route?.id === 'nested-fields' ? <NestedFields /> : route?.id === 'wizard' ? <Wizard /> : route?.id === 'permissions' ? <Permissions /> : route ? <PlaceholderPage route={route} /> : <Stack spacing={2}><Typography component="h1" variant="h3">Page not found</Typography><Alert severity="warning">No route matches {pathname}.</Alert><Button onClick={() => navigate('/')}>Return to dashboard</Button></Stack>;
  return <Box sx={{ display: 'flex', minHeight: '100vh' }}><Drawer variant={permanent ? 'permanent' : 'temporary'} open={permanent || drawerOpen} onClose={() => setDrawerOpen(false)} ModalProps={{ keepMounted: true }} sx={{ '& .MuiDrawer-paper': { width: drawerWidth, boxSizing: 'border-box' } }}>{nav}</Drawer><Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0, ml: permanent ? `${drawerWidth}px` : 0 }}><AppBar position="sticky" color="inherit" elevation={0} sx={{ borderBottom: 1, borderColor: 'divider' }}><Toolbar sx={{ gap: 2 }}><IconButton aria-label="Open navigation" onClick={() => setDrawerOpen(true)}>ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹ÃƒÆ’Ã¢â‚¬Â¦ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°</IconButton><Box sx={{ minWidth: 150 }}><Typography variant="caption" color="text.secondary">Playground / {route?.title ?? 'Not found'}</Typography><Typography variant="h6">{route?.title ?? 'Not found'}</Typography></Box><TextField value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search demos" size="small" inputProps={{ 'aria-label': 'Search demos' }} sx={{ flex: 1, maxWidth: 360 }} /><Stack direction="row" sx={{ display: { xs: 'none', md: 'flex' } }}>{(['desktop','tablet','mobile'] as const).map((item) => <Button key={item} size="small" variant={viewport === item ? 'contained' : 'text'} onClick={() => setViewport(item)}>{item}</Button>)}</Stack><IconButton aria-label="Toggle color theme" onClick={toggleMode}>{mode === 'light' ? 'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â' : 'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¹Ãƒâ€¦Ã¢â‚¬Å“'}</IconButton><Button href="/docs/" sx={{ display: { xs: 'none', sm: 'inline-flex' } }}>Docs</Button><Button href="https://github.com/lourthuxavierm/dynamic-forms" target="_blank" rel="noreferrer" sx={{ display: { xs: 'none', xl: 'inline-flex' } }}>Repository</Button></Toolbar></AppBar><Box component="main" sx={{ p: { xs: 2, md: 3 }, overflow: 'auto' }}><Box sx={{ width: widths[viewport], maxWidth: '100%', mx: 'auto', transition: 'width 180ms ease' }}><PlaygroundErrorBoundary><Suspense fallback={<Stack alignItems="center" sx={{ p: 6 }}><CircularProgress aria-label="Loading demo" /></Stack>}>{page}</Suspense></PlaygroundErrorBoundary></Box></Box></Box></Box>;
}
