...
Run `pnpm --filter @lourthuxavierm/dynamic-forms-react-html performance` on a production build. The command fails when the `core` entry reaches 10 KB gzip, the independently importable text control reaches 2 KB gzip, or synthetic keystroke work reaches 16 ms.

The root entry remains a compatibility convenience containing the complete default registry. Production applications should import registry primitives from `@lourthuxavierm/dynamic-forms-react-html/core` and controls from the `controls/*` entry points. Heavy controls can be code-split by merging `createLazyHtmlRegistry()` into the application registry; `HtmlFieldRenderer` supplies the Suspense boundary.
