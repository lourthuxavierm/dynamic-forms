...
try {
  ...
  runPnpm(['--filter', '@dynamic-forms/zod', 'typecheck']);
  runPnpm(['--filter', '@dynamic-forms/zod', 'test']);
  runPnpm(['--filter', '@dynamic-forms/zod...', 'build']);
  ...
  const output = runPnpm([
    '--filter', '@dynamic-forms/zod', 'pack', '--json', '--pack-destination', verificationRoot,
  ]);
  ...
  assert.equal(manifest.name, '@dynamic-forms/zod');
  assert.equal(manifest.publishConfig?.access, 'public');
  assert.equal(manifest.sideEffects, false);
  assert.equal(manifest.peerDependencies?.zod, '^3.25.5 || ^4.0.0');
  assert.equal(manifest.dependencies?.['@dynamic-forms/core'], manifest.version);
